    ...
    this.setSelectOption = function (optionList, value) {
        ...
        if (optionList.length > 0) {
            for (var i in optionList) {
                option += !website.IsEmpty(optionList[i]) ? "<option value='" + optionList[i]["value"] + "' " + (!website.IsEmpty(value) && optionList[i]["value"] === value.toString() ? "selected" : "") + ">" + optionList[i]["text"] + "</option>" : "";
            }
        }
