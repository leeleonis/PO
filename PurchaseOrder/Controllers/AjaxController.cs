...
namespace PurchaseOrderSys.Controllers
{
    public class AjaxController : BaseController
    {
        ...
        public ActionResult TSkuNumberList(int? draw, int? start, int? length, List<string> Skulist, int? FromWID, string SID, int? ToWID)
        {
            ...
            if (FromWID.HasValue)
            {
                ...
                if (Skulist != null && Skulist.Where(x => !string.IsNullOrWhiteSpace(x)).Any())
                {
                    ...
                    dataList.AddRange(db.PurchaseSKU.Where(x => x.IsEnable && x.PurchaseOrder.IsEnable && x.PurchaseOrder.WarehouseID == FromWID && Skulist.Contains(x.SkuNo)).AsEnumerable().Select(x =>
                        new TranSKUVM
                        {
                            sk = x.SkuNo,
                            SKU = x.SkuNo,
                    ...
                    if (!dataList.Any())
                    {
                        dataList.AddRange(db.TransferSKU.Where(x => x.IsEnable && x.Transfer.IsEnable && x.Transfer.ToWID == FromWID && Skulist.Contains(x.SkuNo)).AsEnumerable().Select(x =>
                        new TranSKUVM
                        {
                            sk = x.SkuNo,
                            SKU = x.SkuNo,
                    ...
                    if (!dataList.Any())
                    {
                        dataList.AddRange(db.RMASerialsLlist.Where(x => x.RMASKU.IsEnable && x.RMASKU.RMA.IsEnable && x.WarehouseID == FromWID && Skulist.Contains(x.NewSkuNo)).AsEnumerable().Select(x =>
                         new TranSKUVM
                         {
                             sk = x.NewSkuNo,
                             SKU = x.NewSkuNo,
        ...
        [HttpPost]
        public ActionResult TSkuNumberListPrice(int id, int val, string SID)
        {
            var odataList = (List<TranSKUVM>)Session["TSkuNumberList" + SID];
            foreach (var item in odataList.Where(x => x.ID == id || x.ck == id))
            {
                    ...
                    if (SerialsLlist.Any())
                    {
                        ...
                        if (POSerialsLlist.Any())
                        {
                            ...
                            try
                            {
                                var SkuNo = "";
                                var SerialsNo = "";
                                if (POSerialsLlist.LastOrDefault().PurchaseSKUID.HasValue)
                                {
                                    SkuNo = POSerialsLlist.LastOrDefault().PurchaseSKU.SkuNo;
                                }
                                else if (POSerialsLlist.LastOrDefault().TransferSKUID.HasValue)
                                {
                        ...
                        else
                        {
                            ...
                            if (WinitSerialsLlist.Any())
                            {
                                ...
                                nInSerials.SerialsLlistC.Add(nSerials);
                                db.SerialsLlist.Add(nInSerials);
                                try
                                {
                                    var SkuNo = "";
                                    var SerialsNo = "";
                                    if (WinitSerialsLlist.LastOrDefault().PurchaseSKUID.HasValue)
                                    {
                                        SkuNo = WinitSerialsLlist.LastOrDefault().PurchaseSKU.SkuNo;
                                    }
                                    else if (WinitSerialsLlist.LastOrDefault().TransferSKUID.HasValue)
                                    {
                ...
                if (QueryOrderTracking.trackingList.Any())
                {
                    ...
                    if ((QueryOrderTracking.trackingList.LastOrDefault().trackingCode == "OWS" || QueryOrderTracking.trackingList.LastOrDefault().trackingCode == "OWA") && !string.IsNullOrWhiteSpace(Transfer.WinitTransfer.WinitOrderNo))//有上架資料才比對
                    {
                        ...
                        foreach (var TransferSKU in TransferSKUList)
                        {
                            ...
                            var SerialsLlist = TransferSKU.SerialsLlist.Where(x => x.IsEnable && x.SerialsType == "TransferOut");//查移出的
                            if (Math.Abs(SerialsLlist.Sum(x => x.SerialsQTY).Value) == actualQuantity)//和實際上架數一樣，直接入庫
                            {
                                foreach (var Serials in SerialsLlist.Where(x => !x.SerialsLlistC.Any(y => y.IsEnable)))//沒有入庫的全部入庫
                                {
        ...
        public void PrintMyFiles(int id, string Page, string key)
        {
            ...
            if (key == "serial")
            {
                ...
                var pdfbyte = Convert.FromBase64String(WinitTransferSKU.ItemBarcodeFile);
                var file = new Neodynamic.SDK.Web.PrintFilePDF(pdfbyte, "SBarcode.pdf");
                file.PagesRange = Page;//要印的頁數
                //file.PrintRotation = Neodynamic.SDK.Web.PrintRotation.Rot90;
                cpj.PrintFileGroup.Add(file);
            }
            else if (key == "box")
            {
                printerName = "Fedex_ZP505";
                var WinitTransfer = db.WinitTransfer.Find(id);
                var pdfbyte = Convert.FromBase64String(WinitTransfer.PrintPackageLabe);
                var file = new Neodynamic.SDK.Web.PrintFilePDF(pdfbyte, "BoxBarcode.pdf");
                cpj.PrintFileGroup.Add(file);
            }
            else if (key == "FedEx")
            {
